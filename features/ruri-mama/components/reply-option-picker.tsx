 ...
 * バッジ用の淡い chip のみ。gold / champagne 系の sanctioned パレットで統一。
 */
const STYLE_TONE: Record<ReplyOptionStyle, { accent: string; chip: string }> = {
  safe: {
    accent: "text-gold-deep",
    chip: "bg-champagne-soft text-gold-deep",
  },
  practical: {
    accent: "text-ink",
...
function OptionCard({
  ...
  return (
    <div
      className={cn(
        "rounded-card border bg-pearl-warm transition-all",
        isSelected
    ...
    >
      ...
      <div className="flex items-center px-3 pt-2.5 pb-1.5">
        <div className={cn("flex items-center gap-1.5 text-label-sm font-semibold", tone.accent)}>
          ...
          <span className="text-[10px] font-normal text-ink-muted">· {option.label}</span>
        </div>
      </div>
      ...
      <div className="px-3 pb-2.5">
        {hasMessage && (
          <div className="flex items-center gap-1 text-[10px] text-ink-muted mb-1">
        ...
        <div
          className={cn(
            ...
            hasMessage ? "bg-champagne-soft/50 border border-gold/15" : "",
          )}
        >
          {primary.body}
        </div>
          ...
          <>
            ...
            {showDetail && (
              <div className="mt-1.5 space-y-1.5 animate-fade-in">
                {details.map((s, idx) => (
                  <div key={idx} className="text-body-sm leading-relaxed text-ink-secondary">
                    {s.name && (
      ...
      <div className="px-3 pb-3">
        <button
          ...
          className={cn(
            ...
            isSelected
              ? "bg-gold-deep text-pearl-warm"
              : locked
                ? "bg-pearl-soft text-ink-muted cursor-not-allowed"
                : "bg-gold-deep text-pearl-warm shadow-warm hover:opacity-90",
          )}
        >
          {isSelected ? (
  ...
  return (
    <button
      ...
      className={cn(
        ...
        disabled
          ? "bg-pearl-soft text-ink-muted cursor-not-allowed"
          : "bg-champagne-soft text-gold-deep hover:bg-champagne-soft/80",
      )}
    >
      <Wand2 size={11} />
      この文面をブラッシュアップする

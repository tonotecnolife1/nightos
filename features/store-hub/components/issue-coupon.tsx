    ...
    <Card className="!bg-pearl-warm !border-pearl-soft p-4 space-y-3">
      ...
      {success ? (
        ...
        <button
          ...
          onClick={handleIssue}
          disabled={pending}
          className="w-full h-9 rounded-btn bg-gradient-amethyst text-pearl text-[11px] font-medium shadow-soft disabled:opacity-50 active:scale-95 flex items-center justify-center gap-1"
        >
          <Gift size={12} />
          {pending ? "発行中…" : "クーポンを発行"}
        </button>
